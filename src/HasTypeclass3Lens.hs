...
module HasTypeclass3Lens where
...
modify :: (MonadReader env m, HasBalance env (TVar Int), MonadIO m) => (Int -> Int) -> m ()
modify f = do
  env <- ask
  liftIO . atomically $ modifyTVar' (env^.balance) f

logSomething :: (MonadReader env m, HasLog env (String -> IO ()), MonadIO m) => String -> m ()
logSomething msg = do
  env <- ask
  liftIO $ (env^.log) msg
